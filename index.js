...
const vehicle = new Vehicle("Audi", "A6", 2018, 30000)
...
function Car(brand, model, year, mileage, fuelType, speed) {
  ...
  Car.prototype.toString = function () {
    return `${this.brand} ${this.model} (${this.year}) - ${this.fuelType}, швидкість: ${this.speed} км/год`;
  }

  Car.prototype.drive = function (kilometers) {
    const totalKilometers = kilometers + this.mileage;

    return `Подорожуємо ${totalKilometers} кілометрів у ${this.brand} ${this.model}`
  }
}
...
function Truck(
  ...
  Truck.prototype.specific = function (number) {
    ...
    } else { return `нормальне навантаження... ` }
  }
}
...
///    Що ЦЕ ЗА МЕТОД МАЄ БУТИ ??

// Викликаємо метод tow з вагою більшою за towingCapacity
console.log(truck.specific(1000))
console.log(truck.specific(100000))
...
const driveFunction = car.drive.bind(car, 100);
console.log(driveFunction());
